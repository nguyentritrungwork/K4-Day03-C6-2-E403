...
def search_properties(
    ...
    try:
        if not location or not isinstance(location, str) or not location.strip():
            return "LỖI: Vui lòng nhập khu vực/địa điểm muốn tìm kiếm (Ví dụ: 'Quận 1', 'Thủ Đức')."
        ...
        return f"LỖI HỆ THỐNG: Không thể tìm kiếm bất động sản. Chi tiết lỗi: {str(e)}"
...
def get_property_details(property_id: str) -> str:
    ...
    try:
        ...
        if not property_id or not isinstance(property_id, str):
            return "LỖI: Mã bất động sản không hợp lệ. Vui lòng cung cấp mã hợp lệ (Ví dụ: 'CH01', 'NT02')."
        ...
        return f"LỖI HỆ THỐNG: Không thể xem chi tiết bất động sản. Chi tiết lỗi: {str(e)}"
...
def check_viewing_availability(property_id: str, date: str) -> str:
    ...
    try:
        if not property_id or not isinstance(property_id, str):
            return "LỖI: Mã bất động sản không hợp lệ. Vui lòng cung cấp mã hợp lệ (Ví dụ: 'CH01')."
        if not date or not isinstance(date, str):
            return "LỖI: Ngày không hợp lệ. Vui lòng cung cấp ngày theo định dạng DD-MM-YYYY (Ví dụ: '01-08-2026')."
        # Mock data — mô phỏng dữ liệu trả về
        return (
        ...
        return f"LỖI HỆ THỐNG: Không thể kiểm tra lịch xem nhà. Chi tiết lỗi: {str(e)}"
...
def book_viewing(
    ...
    try:
        ...
        if not property_id or not isinstance(property_id, str):
            return "LỖI: Mã bất động sản không hợp lệ. Vui lòng cung cấp mã hợp lệ (Ví dụ: 'CH01')."
        if not date or not isinstance(date, str):
            return "LỖI: Ngày không hợp lệ. Vui lòng cung cấp ngày theo định dạng DD-MM-YYYY."
        if not time or not isinstance(time, str):
            return "LỖI: Giờ không hợp lệ. Vui lòng cung cấp giờ (Ví dụ: '08:00', '10:00')."
        if not name or not isinstance(name, str) or not name.strip():
            return "LỖI: Họ tên không được để trống. Vui lòng nhập họ tên người đặt lịch."
        if not phone or not isinstance(phone, str) or not phone.strip():
            return "LỖI: Số điện thoại không được để trống. Vui lòng nhập số điện thoại liên hệ."
        ...
        return f"LỖI HỆ THỐNG: Không thể đặt lịch xem nhà. Chi tiết lỗi: {str(e)}"
...
def cancel_viewing(viewing_id: str) -> str:
    ...
    """
    try:
        if not viewing_id or not isinstance(viewing_id, str) or not viewing_id.strip():
            return "LỖI: Mã lịch hẹn không hợp lệ. Vui lòng cung cấp mã lịch hẹn cần hủy (Ví dụ: 'VIEW-CH01-20260801-0800')."
        ...
        return f"LỖI HỆ THỐNG: Không thể hủy lịch xem nhà. Chi tiết lỗi: {str(e)}"
...
def get_my_viewings(phone: str) -> str:
    ...
    try:
        if not phone or not isinstance(phone, str) or not phone.strip():
            return "LỖI: Số điện thoại không hợp lệ. Vui lòng nhập số điện thoại đã dùng để đặt lịch (Ví dụ: '0901234567')."
        ...
        return f"LỖI HỆ THỐNG: Không thể lấy danh sách lịch hẹn. Chi tiết lỗi: {str(e)}"
